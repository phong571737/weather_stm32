 ...
 * @version V.1.0.0
 *
 * Licensing: https://github.com/maudeve-it/W25Qxxx_SPI_FLASH_STM32/blob/main/LICENSE
 *
 *********************************************
 ...
 ********************************************
 *
 * Install and use this library following instruction on: https://github.com/maudeve-it/W25Qxxx_SPI_FLASH_STM32
 *
 * Put init function in you main() USER CODE BEGIN 2:
 *
 *  if (!Flash_Init()) {};
 *
 *  that's all
...
/**************************
 **************************/
uint8_t Flash_ReadSR(uint8_t buffer){
	Flash_Select();
...
uint8_t buffer[5];
	...
	// dataSize is 32 bit, HAL_SPI_Receive handles 16bit transfers, so I have to loop...
	while (dataSize) {
		data_to_transfer = ((dataSize>EXT_FLASH_READING_PAGE) ? (uint16_t)EXT_FLASH_READING_PAGE : (uint16_t)dataSize);
		Flash_Receive(data, data_to_transfer);
		data+=(uint32_t)data_to_transfer;
		dataSize-=(uint32_t)data_to_transfer;
	}
	Flash_UnSelect();
}
...
uint8_t buffer[4];
uint32_t quota; // counter of the transferred data in a multi-page writing request
uint32_t inpage_addr;
	...
	//loop here inside, while more than 1 Flash page need to be written...
...
/**********************************
 * @BRIEF	Resets the Block protecting
 * 			bits in SR1 register
 *********************************/
void Flash_ResetSR1(){
uint8_t buffer[2];
...
/*********************************
 * @BRIEF	testing chip alive and kicking reading SFDP
 * 	 		record ("Serial Flash Discoverable Parameter"
 * 			as per Jedec standard), it must return a
 * 			string beginning with "SFDP"
 ...
 * @RETURN	1 	init OK (error is 0)
 * 			0	Winbond Flash not found (error==1 means there is no a I2C Jedec device, error==2 there is a device but it is no a winbond memory)
 ******************************************************************/
uint8_t Flash_Init(uint8_t *error){
uint32_t JedecID;
	HAL_Delay(6);							// if init is called on system startup: 5 ms (tPUW) required after power-up to be fully available
	Flash_Reset();							// try to reset the device, even if not yet tested presence
	HAL_Delay(6);							// if init is called on system startup: 5 ms (tPUW) required after power-up to be fully available
	*error=0;
	if (!Flash_TestAvailability())			// is there a serial flash? if not
		*error=1;
	JedecID=Flash_ReadJedecID() ;			//select the memSize byte
	if (((JedecID >> 16) & 0XFF) != 0xEF)  	// is it a Winbond device? if not
		*error=2;
...
/******************************************************************
 * @BRIEF	sends a reset command sequence over SPI
 ******************************************************************/
void Flash_Reset(){
uint8_t command;
